                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('masyarakat.jadi-petugas.submit') }}" enctype="multipart/form-data">
                        ...
                        <div class="alert alert-light border mb-4">
                            ...
                            <div class="mt-3">
                                <small class="text-danger">* Dengan mengisi form ini, Anda menyetujui ketentuan yang berlaku.</small>
                            </div>
                        </div>
                        ...
                        <div id="mainForm" style="opacity: 0.5; pointer-events: none;">
                            ...
                            <div class="mb-4">
                                ...
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <select class="form-control @error('provinsi_id') is-invalid @enderror" name="provinsi_id" id="provinsi">
                                    ...
                                    <select class="form-control @error('kabupaten_id') is-invalid @enderror" name="kabupaten_id" id="kabupaten">
                                        <option value="">Pilih Kabupaten/Kota</option>
                                        @if(old('kabupaten_id'))
                                            <option value="{{ old('kabupaten_id') }}" selected>{{ old('kabupaten_name') }}</option>
                                        @endif
                                    </select>
                                    @error('kabupaten_id')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                ...
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <select class="form-control @error('kecamatan_id') is-invalid @enderror" name="kecamatan_id" id="kecamatan">
                                            <option value="">Pilih Kecamatan</option>
                                            @if(old('kecamatan_id'))
                                                <option value="{{ old('kecamatan_id') }}" selected>{{ old('kecamatan_name') }}</option>
                                            @endif
                                        </select>
                                        @error('kecamatan_id')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                    ...
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <select class="form-control @error('desa_id') is-invalid @enderror" name="desa_id" id="desa">
                                            <option value="">Pilih Desa/Kelurahan</option>
                                            @if(old('desa_id'))
                                                <option value="{{ old('desa_id') }}" selected>{{ old('desa_name') }}</option>
                                            @endif
                                        </select>
                                        @error('desa_id')
                                            <div class="invalid-feedback">{{ $message }}</div>
                           ...
                           <!-- Data Confirmation Section -->
                            <div class="mb-4">
                                <div class="alert border" style="background-color: rgba(41, 158, 99, 0.1); border-color: #299E63;">
                                    <div class="form-check d-flex align-items-start">
                                        <input class="form-check-input me-3 mt-1" type="checkbox" id="dataConfirmation" onchange="toggleSubmitButton()" style="transform: scale(1.2);">
                                        <label class="form-check-label" for="dataConfirmation">
                                            <strong>Dengan ini, saya menyatakan bahwa data yang saya berikan benar dan saya menyetujui semua ketentuan yang berlaku.</strong>
                                        </label>
                                    </div>
                                </div>
                            </div> 
                            ...
                            <!-- Submit Button -->
                            <div class="d-grid">
                                <button type="submit" class="btn btn-lg text-white" style="background-color: #299E63" id="submitBtn">
                                    Kirim Pendaftaran
                                </button>
                            </div>
                        </div>
...
<script>
    $(document).ready(function() {
        ...
        function loadRegencies(provinceId, selectedId = null) {
            ...
            $.ajax({
                url: '{{ route("masyarakat.get.regencies", ["province_id" => ":province_id"]) }}'.replace(':province_id', provinceId),
                type: 'GET',
                dataType: 'json',
        ...
        function loadDistricts(regencyId, selectedId = null) {
            ...
            $.ajax({
                url: '{{ route("masyarakat.get.districts", ["regency_id" => ":regency_id"]) }}'.replace(':regency_id', regencyId),
                type: 'GET',
                dataType: 'json',
                success: function(data) {
        ...
        function loadVillages(districtId, selectedId = null) {
            ...
            $.ajax({
                url: '{{ route("masyarakat.get.villages", ["district_id" => ":district_id"]) }}'.replace(':district_id', districtId),
                type: 'GET',
                dataType: 'json',
                cache: false,
                success: function(data) {
